...
- **멀티 프로바이더**: Gemini, OpenAI, OpenRouter, fal.ai, BytePlus 중 원하는 백엔드를 선택합니다.
...
`cmd/ppgen`은 같은 파이프라인을 CLI에서 실행합니다. 런 간 캐릭터 정체성을 고정하려면 base 이미지를
한 번 확정한 뒤 이후 상태나 방향 세트 생성에 재사용합니다.
...
`-seed N`은 지원 프로바이더(`fal`, `gemini`, `openrouter`)에 결정적 샘플링을 요청합니다. 다만
정체성 고정의 1차 수단은 실제 이미지를 참조로 쓰는 `-base`입니다.
